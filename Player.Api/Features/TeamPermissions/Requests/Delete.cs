...
public class Delete
{
    [DataContract(Name = "DeleteTeamPermissionCommand")]
    ...
    public class Endpoint : IEndpoint
    {
        ...
        {
            return [
                group.MapDelete("team-permissions/{id}", TypedHandler)
                    .WithName("deleteTeamPermission")
                    .WithDescription("Deletes a TeamPermission with the specified id.")
                    .WithSummary("Deletes a TeamPermission.")
            ];
        }
    ...
    public class Handler(IPlayerAuthorizationService authorizationService, PlayerContext db) : BaseHandler<Command>
    {
        ...
        {
            var permissionToDelete = await db.TeamPermissions.SingleOrDefaultAsync(t => t.Id == request.Id, cancellationToken);

            if (permissionToDelete == null)
                throw new EntityNotFoundException<TeamPermissionModel>();

            if (permissionToDelete.Immutable)
                throw new ForbiddenException("Cannot delete a Read-Only TeamPermissionModel");

            db.TeamPermissions.Remove(permissionToDelete);
