...
public class GetAll
{
    [DataContract(Name = "GetTeamPermissionsQuery")]
    ...
    public class Endpoint : IEndpoint
    {
        ...
        {
            return [
                group.MapGet("team-permissions", TypedHandler)
                    .WithName("getTeamPermissions")
                    .WithDescription("Returns a list of all of the TeamPermissions in the system.")
                    .WithSummary("Gets all TeamPermissions in the system.")
            ];
        }

        async Task<Ok<TeamPermissionModel[]>> TypedHandler(IMediator mediator, CancellationToken cancellationToken)
        {
    ...
    public class Handler(IPlayerAuthorizationService authorizationService, PlayerContext db, IMapper mapper) : BaseHandler<Query, TeamPermissionModel[]>
    {
        ...
        {
            return await db.TeamPermissions
