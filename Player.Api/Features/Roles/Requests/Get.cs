...
public class Get
{
    [DataContract(Name = "GetRoleQuery")]
    ...
    public class Endpoint : IEndpoint
    {
        ...
        {
            return [
                group.MapGet("roles/{id}", TypedHandler)
                    .WithName("getRole")
                    .WithDescription("Returns the Role with the id specified.")
                    .WithSummary("Gets a specific Role by id.")
            ];
        }

        async Task<Ok<Role>> TypedHandler(Guid id, IMediator mediator, CancellationToken cancellationToken)
        {
